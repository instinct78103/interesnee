const projects = [
  {
    id: 2,
    name: 'Trivver',
    slug: 'trivver',
    mainImage: {
      x1: '/images/projects/Trivver/Trivver_1920px.jpg',
      x2: '/images/projects/Trivver/Trivver_1920px.jpg',
      placeholder: '/images/projects/Trivver/Trivver_1920px@placeholder.jpg',
    },
    thumbnail: {
      x1: '/images/projects/Trivver/iPad.jpg',
      x2: '/images/projects/Trivver/iPad@2x.jpg',
      placeholder: '/images/projects/Trivver/trivver-projects@placeholder.jpg',
      webp: '/images/projects/Trivver/iPad.webp',
    },
    description: {
      issue:
        'Любите компьютерные игры? Как насчет таргетированной рекламы внутри игр? Скажем, вы фанат кока-колы и не ' +
        'любите пепси - так почему бы не показывать вам ее в виртуальном мире? Наш клиент взял эту идею за основу ' +
        'успешного стартапа и привлек нашу команду для ее реализации.',
      process:
        'Наши лучшие технические архитекторы собрались вместе для решения вопросов, ' +
        'связанных с масштабируемостью и использованием решения. После того как план был готов, ' +
        'команда Unity, PHP и мобильных разработчиков взялась за дело!',
      result:
        'Первый релиз был суперуспешным и привлек несколько раундов инвестиций плюс ' +
        'много идей по развитию. AR/VR/Блокчейн - все это есть у нас!',
    },
    shortDescription:
      'Любите компьютерные игры? Как насчет таргетированной рекламы внутри игр? Скажем, вы фанат кока-колы и не ' +
      'любите пепси - так почему бы не показывать вам ее в виртуальном мире? Наш клиент взял эту идею за основу ' +
      'успешного стартапа и привлек нашу команду для ее реализации.',
    images: [
      {
        x1: '/images/projects/Trivver/bg_1@1x.jpg',
        x2: '/images/projects/Trivver/bg_1@1x.jpg',
        placeholder: '/images/projects/Trivver/bg_1@placeholder.jpg',
      },
      {
        x1: '/images/projects/Trivver/bg_2@1x.jpg',
        x2: '/images/projects/Trivver/bg_2@1x.jpg',
        placeholder: '/images/projects/Trivver/bg_2@placeholder.jpg',
      },
      {
        x1: '/images/projects/Trivver/bg_3@1x.jpg',
        x2: '/images/projects/Trivver/bg_3@1x.jpg',
        placeholder: '/images/projects/Trivver/bg_3@placeholder.jpg',
      },
    ],
  },
  {
    id: 3,
    name: 'VR Puzzle Box',
    slug: 'vr-puzzle-box',
    mainImage: {
      x1: '/images/projects/VRPuzzleBox/VRPuzzleBox-head-new.png',
      x2: '/images/projects/VRPuzzleBox/VRPuzzleBox-head-new.png',
      placeholder:
        '/images/projects/VRPuzzleBox/VRPuzzleBox-head@placeholder.jpg',
    },
    thumbnail: {
      x1: '/images/projects/VRPuzzleBox/VRPuzzleBox-projects.jpg',
      x2: '/images/projects/VRPuzzleBox/VRPuzzleBox-projects.jpg',
      placeholder:
        '/images/projects/VRPuzzleBox/VRPuzzleBox-projects@placeholder.jpg',
      webp: '/images/projects/VRPuzzleBox/VRPuzzleBox-projects.webp',
    },
    description: {
      issue:
        'В 2019 году наша компания принимала участие в конференции DUMP, специально для которой было разработано ' +
        'приложение - VR Puzzle Box. По результатам презентации созданный прототип игры получил положительные отзывы ' +
        'от участников. Прислушавшись к фидбэку пользователей, мы расширили  функционал игры и разместили ее в ' +
        '<a target="_blank" rel="noopener noreferrer" href="https://store.steampowered.com/app/1125730/VR_Puzzle_Box/">steam</a>.',
      process:
        'Одной из сложных задач оказалась физика взаимодействия с объектами виртуальной реальности или, ' +
        'другими словами, как научить пользователя взаимодействовать с несуществующими объектами. ' +
        'Как взять предмет, как его повернуть, как его положить, не нарушая законы физики? ' +
        'Для этого была реализована физическая модель с пружинами, которые разрываются, ' +
        'если ты нарушаешь эти законы. ' +
        '<br> <br> <span class="intend"></span>' +
        ' Еще одним интересным моментом стала разработка механизм подъема и опускания игровой поверхности, ' +
        'так как мы разрабатывали игру, в которой пользователь должен был играть стоя, ' +
        'и в процессе тестирования поняли, что рост игроков у всех разный.',
      result:
        'Результатом стала разработанная игра, ' +
        'в которой пользователь должен расположить объемные детали головоломки на игровом поле таким образом, ' +
        'чтобы получилось плоское двухмерное изображение. Игра имеет несколько уровней сложности и режимов. ' +
        '<br> <br> <span class="intend"></span>' +
        'Данное приложение можно скачать в ' +
        '<a target="_blank" rel="noopener noreferrer" href="https://store.steampowered.com/app/1125730/VR_Puzzle_Box/">steam</a>.',
    },
    shortDescription:
      'В 2019 году наша компания принимала участие в конференции DUMP, специально для которой было разработано приложение - VR Puzzle Box. По результатам презентации созданный прототип игры получил положительные отзывы от участников.',
    images: [
      {
        x1: '/images/projects/VRPuzzleBox/1.jpg',
        x2: '/images/projects/VRPuzzleBox/1.jpg',
      },
      {
        x1: '/images/projects/VRPuzzleBox/2.jpg',
        x2: '/images/projects/VRPuzzleBox/2.jpg',
      },
      {
        x1: '/images/projects/VRPuzzleBox/3.jpg',
        x2: '/images/projects/VRPuzzleBox/3.jpg',
      },
      {
        x1: '/images/projects/VRPuzzleBox/4.jpg',
        x2: '/images/projects/VRPuzzleBox/4.jpg',
      },
      {
        x1: '/images/projects/VRPuzzleBox/5.jpg',
        x2: '/images/projects/VRPuzzleBox/5.jpg',
      },
      {
        x1: '/images/projects/VRPuzzleBox/6.jpg',
        x2: '/images/projects/VRPuzzleBox/6.jpg',
      },
    ],
    videos: [
      {
        poster: '/images/projects/VRPuzzleBox/VRPuzzleBox@video-preview.jpg',
        sources: [
          {
            src: '/videos/projects/VRPuzzleBox/1.mp4',
            type: 'video/mp4',
          },
          {
            src: '/videos/projects/VRPuzzleBox/1.webm',
            type: 'video/webm',
          },
        ],
      },
    ],
  },
  {
    id: 4,
    name: 'August McGregor',
    slug: 'august-mcgregor',
    mainImage: {
      x1: '/images/projects/August%20McGregor/2_1920.png',
      x2: '/images/projects/August%20McGregor/2_1920@2x.png',
      placeholder: '/images/projects/August%20McGregor/2_1920@placeholder.jpg',
    },
    description: {
      issue:
        'Разработать маркетинговый промо-сайт для модельера одежды Коннора МакГрегора, да еще и перед его важным боем с непобедимым Флойдом Мэйуэзером!',
      process:
        'Несмотря на то, что на сайте всего одна страница, ' +
        'каждая деталь была точно отполирована и выверена. ' +
        'Добавьте сюда еще недельный дедлайн и получите работу, ' +
        'которую может сделать только наша команда!',
      result:
        'Бой прошел успешно. Пусть Коннор и не победил, ' +
        'но его партнерский сайт получил много заявок. Следите за обновлениями!',
    },
    images: [
      {
        x1: '/images/projects/August%20McGregor/1.jpg',
        x2: '/images/projects/August%20McGregor/1.jpg',
        placeholder: '/images/projects/August%20McGregor/1@placeholder.jpg',
      },
      {
        x1: '/images/projects/August%20McGregor/2.jpg',
        x2: '/images/projects/August%20McGregor/2@2x.jpg',
        placeholder: '/images/projects/August%20McGregor/2@placeholder.jpg',
      },
      {
        x1: '/images/projects/August%20McGregor/3.jpg',
        x2: '/images/projects/August%20McGregor/3@2x.jpg',
        placeholder: '/images/projects/August%20McGregor/3@placeholder.jpg',
      },
    ],
    thumbnail: {
      x1: '/images/projects/August%20McGregor/2_1920.png',
      x2: '/images/projects/August%20McGregor/2_1920@2x.png',
    },
  },
  {
    id: 5,
    name: 'Medic Clipboard',
    slug: 'medic-clipboard',
    mainImage: {
      x1: '/images/projects/Medic%20Clipboard/3_1920px.png',
      x2: '/images/projects/Medic%20Clipboard/3_1920px@2x.png',
      placeholder:
        '/images/projects/Medic%20Clipboard/3_1920px@placeholder.jpg',
    },
    description: {
      issue:
        'Что если детальная медицинская информация с места выезда бригад скорой помощи будет поступать на пульт ' +
        'мгновенно - сколько жизней можно спасти? Наш клиент - инновационная компания, которая поставила перед собой задачу ' +
        'перевода бумажной работы парамедиков в цифровой формат и интеграции этого решения с диспетчерскими центрами. ',
      process:
        'Задача, признаться, нелегкая ведь приложение должно быть всегда доступно и понятно для медицинской ' +
        'бригады - на счету каждая секунда. Наши ребята работали над созданием простого интерфейса максимально похожего ' +
        'на существующие государственные формы, которые использовались при бумажной работе. ',
      result:
        'Приложение активно работает в округе Лос-Анджелеса - надеемся, что мы спасли не одну жизнь! Ведь теперь ' +
        'больницы получают всю информацию о прибывающем пациенте еще до момента приезда бригады, а значит могут ' +
        'приготовить все необходимое для оказания помощи!',
    },
    images: [
      {
        x1: '/images/projects/Medic%20Clipboard/1.jpg',
        x2: '/images/projects/Medic%20Clipboard/1@2x.jpg',
        placeholder: '/images/projects/Medic%20Clipboard/1@placeholder.jpg',
      },
      {
        x1: '/images/projects/Medic%20Clipboard/2.jpg',
        x2: '/images/projects/Medic%20Clipboard/2@2x.jpg',
        placeholder: '/images/projects/Medic%20Clipboard/2@placeholder.jpg',
      },
      {
        x1: '/images/projects/Medic%20Clipboard/3.jpg',
        x2: '/images/projects/Medic%20Clipboard/3@2x.jpg',
        placeholder: '/images/projects/Medic%20Clipboard/3@placeholder.jpg',
      },
    ],
    thumbnail: {
      x1: '/images/projects/Medic%20Clipboard/1.jpg',
      x2: '/images/projects/Medic%20Clipboard/1@2x.jpg',
    },
  },
  {
    id: 6,
    name: 'Microchip',
    slug: 'microchip',
    mainImage: {
      x1: '/images/projects/Microchip/4_1920px.jpg',
      x2: '/images/projects/Microchip/4_1920px@2x.png',
      placeholder: '/images/projects/Microchip/4_1920px@placeholder.jpg',
    },
    thumbnail: {
      x1: '/images/projects/Microchip/Iot.jpg',
      x2: '/images/projects/Microchip/Iot@2x.jpg',
    },
    description: {
      issue:
        'Один из крупнейших мировых производителей микропроцессоров обратился к нам за помощью в создании ' +
        'интегрированного IoT решения для прототипирования проектов на базе контроллеров Microchip.',
      process:
        'Наши инженеры разработали прошивку для прототипа, соединили его с облачными сервисами и создали ' +
        'мобильное решение для получения информации с сенсоров.',
      result:
        'Компания Microchip активно продвигает решение через семинары, тренинги, конференции и подключает ' +
        'нас для разработки индивидуальных проектов. ',
    },
    shortDescription:
      'Мы разработали полное IoT решение на базе партнерства с одним из крупнейших провайдеров мобильной связи (AT&T).',
    images: [
      {
        x1: '/images/projects/Microchip/1.jpg',
        x2: '/images/projects/Microchip/1.jpg',
        placeholder: '/images/projects/Microchip/1@placeholder.jpg',
      },
      {
        x1: '/images/projects/Microchip/2.jpg',
        x2: '/images/projects/Microchip/2@2x.jpg',
        placeholder: '/images/projects/Microchip/2@placeholder.jpg',
      },
      {
        x1: '/images/projects/Microchip/3.jpg',
        x2: '/images/projects/Microchip/3@2x.jpg',
        placeholder: '/images/projects/Microchip/3@placeholder.jpg',
      },
    ],
  },
  {
    id: 7,
    name: 'National Signal',
    slug: 'national-signal',
    mainImage: {
      x1: '/images/projects/National%20Signal/5_1920px.jpg',
      x2: '/images/projects/National%20Signal/5_1920px@2x.png',
      placeholder:
        '/images/projects/National%20Signal/5_1920px@placeholder.jpg',
    },
    thumbnail: {
      x1: '/images/projects/National%20Signal/Iot.jpg',
      x2: '/images/projects/National%20Signal/Iot@2x.jpg',
    },
    description: {
      issue:
        'Компания National Signal решила пойти в сторону облачных решений для диагностики и обновления информации на ' +
        'мобильных информационных экранах, которые размещаются вдоль дорог на территории США. ',
      process:
        'Мы разработали IoT решение на базе облачных сервисов одного из крупнейших провайдеров мобильной связи (AT&T). ' +
        'Мобильный модем взаимодействует с контроллером экрана и организует клиент-серверное взаимодействие. ' +
        'Все протоколы коммуникаций соответствуют стандарту Департамента Транспорта США.',
      result:
        'Данная система внедрена и используется по всей территории страны. Возможность удаленного обновления кода без ' +
        'выезда специалистов на место установки экрана позволила оперативно решать проблемы, найденные в процессе ' +
        'запуска и эксплуатации. Клиент остался очень доволен результатом! Ведь теперь можно мониторить весь парк ' +
        'мобильных информационных экранов, не отходя от компьютера, и обновлять информацию, отображаемую на них, ' +
        'в течение нескольких секунд.',
    },
    shortDescription:
      'Компания National Signal решила пойти в сторону облачных решений для диагностики и обновления ' +
      'информации на мобильных табло на территории США.',
    images: [
      {
        x1: '/images/projects/National%20Signal/1.jpg',
        x2: '/images/projects/National%20Signal/1.jpg',
        placeholder: '/images/projects/National%20Signal/1@placeholder.jpg',
      },
      {
        x1: '/images/projects/National%20Signal/2.jpg',
        x2: '/images/projects/National%20Signal/2@2x.jpg',
        placeholder: '/images/projects/National%20Signal/2@placeholder.jpg',
      },
      {
        x1: '/images/projects/National%20Signal/3.jpg',
        x2: '/images/projects/National%20Signal/3@2x.jpg',
        placeholder: '/images/projects/National%20Signal/3@placeholder.jpg',
      },
    ],
  },
  {
    id: 8,
    name: 'Grabber',
    slug: 'grabber',
    mainImage: {
      x1: '/images/projects/Grabber/grabber_mainbanner_1920px.jpg',
      x2: '/images/projects/Grabber/grabber_mainbanner_1920px.jpg',
      placeholder: '/images/projects/Grabber/grabber_projectpage_990px.jpg',
    },
    description: {
      issue:
        'В рамках летней практики, проходившей в 2021 году по направлению Unity, студентам было предложено реализовать ' +
        'проект, в котором бы была реализована манипуляция с помощью контролеров.' +
        '<br> <br> <span class="intend"></span>' +
        'После обсуждения с командой, за основу была взята 3D модель промышленного ангара с краном-подъемником. ' +
        'Далее разработчики добавили недостающие 3D модели и проработали игровой процесс. В результате получился ' +
        'VR-проект Grabber, который выложили в ' +
        '<a target="_blank" rel="noopener noreferrer" href="https://www.oculus.com/experiences/quest/4741188125905583?ranking_trace=104878301516[…]1188125905583_SKYLINEWEB_5fcb02fb-37ac-4006-bbe5-86fa650f4251">oculus</a>.',
      process:
        'Одной из самых сложных задач, с которой ребята столкнулись во время разработки - необходимость создания ' +
        'правдоподобного взаимодействия между объектами, учитывая физику и соблюдая баланс массы и жесткости сцепления ' +
        'каждой подвижной части. Путем калибровки и поиска вариантов удалось прийти к нужному результату.' +
        '<br> <br> <span class="intend"></span>' +
        'Второй, но не менее важной задачей, с которой столкнулись разработчики - это оптимизация производительности, ' +
        'т.к. из-за большого количества объектов возрастала нагрузка на CPU. Была проведена достаточно большая работа, ' +
        'чтобы это исправить.' +
        '<br> <br> <span class="intend"></span>' +
        'На завершающем этапе разработки к проекту подключились 3D художники, которые довели модели и освещение проекта ' +
        'до хорошего качества с сохранением высокой производительности. Это позволило сгустить атмосферу за счет теней ' +
        'и бликов, сохранив картинку объемной и живой.' +
        '<br> <br> <span class="intend"></span>' +
        'И в завершении, чтобы сделать управление рычагами более реалистичным, мы создали модель подставок-держателей,' +
        'а один из сотрудников напечатал их на 3D-принтере.',
      result:
        'Результатом стала разработанная игра, в которой пользователь должен захватить бочки с конвейера и ' +
        'утилизировать в измельчителе.' +
        '<br> <br> <span class="intend"></span>' +
        'Данное приложение доступно для скачивания владельцами VR-шлемов Oculus в ' +
        '<a target="_blank" rel="noopener noreferrer" href="https://www.oculus.com/experiences/quest/4741188125905583?ranking_trace=104878301516[…]1188125905583_SKYLINEWEB_5fcb02fb-37ac-4006-bbe5-86fa650f4251">oculus</a>.',
    },
    images: [
      {
        x1: '/images/projects/Grabber/grabber_screenshot_1.jpg',
        x2: '/images/projects/Grabber/grabber_screenshot_1.jpg',
      },
      {
        x1: '/images/projects/Grabber/grabber_screenshot_2.jpg',
        x2: '/images/projects/Grabber/grabber_screenshot_2.jpg',
      },
      {
        x1: '/images/projects/Grabber/grabber_screenshot_3.jpg',
        x2: '/images/projects/Grabber/grabber_screenshot_3.jpg',
      },
    ],
    thumbnail: {
      x1: '/images/projects/Grabber/grabber_projectpage_990px.jpg',
      x2: '/images/projects/Grabber/grabber_projectpage_990px.jpg',
    },
    videos: [
      {
        poster: '/images/projects/Grabber/grabber_projectpage_990px.jpg',
        sources: [
          {
            src: '/videos/projects/Grabber/Grabber_portfolio_01.mp4',
            type: 'video/mp4',
          },
          {
            src: '/videos/projects/Grabber/Grabber_portfolio_01.webm',
            type: 'video/webm',
          },
        ],
      },
    ],
  },
];

const projectsIds = [2, 3];

const projectsSlides = projects.filter(p => projectsIds.includes(p.id));

export { projectsSlides, projects };
